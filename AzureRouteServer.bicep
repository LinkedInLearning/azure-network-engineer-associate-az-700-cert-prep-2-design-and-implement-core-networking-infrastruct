...
param vnetNew_or_Existing string = 'Existing'

@description('Route Server subnet IP prefix MUST be within vnet IP prefix address space.')
param routeServerSubnetIpPrefix string = '10.0.0.64/26'
...
param peerIp string = '192.168.0.1'
...
resource vnet 'Microsoft.Network/virtualNetworks@2020-05-01' = if (vnetNew_or_Existing == 'Existing') {
  name: vnetName
  location: location
  properties: {
    addressSpace: {
